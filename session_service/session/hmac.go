...
func (tk *HashToken) Create(sessionID, userID string, tokenExpTime int64) (string, error) {
	h := hmac.New(sha256.New, []byte(tk.Secret))
	data := fmt.Sprintf("%s:%s:%d", sessionID, userID, tokenExpTime)
	h.Write([]byte(data))
	token := hex.EncodeToString(h.Sum(nil)) + ":" + strconv.FormatInt(tokenExpTime, 10)
	return token, nil
}

func (tk *HashToken) Check(sessionID, userID string, inputToken string) (bool, error) {
	...
	h := hmac.New(sha256.New, []byte(tk.Secret))
	data := fmt.Sprintf("%s:%s:%d", sessionID, userID, tokenExp)
	h.Write([]byte(data))
